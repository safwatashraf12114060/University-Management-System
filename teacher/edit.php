    ...
    if ($values["name"] === "" || $values["dept_id"] === "") {
        ...
        if ($up) {
            ...
            $teacherMessage = $teacherLabel !== ""
                ? "Teacher '" . $teacherLabel . "' was updated."
                : "Teacher details were updated.";
            umsLogActivity($conn, "teacher_update", $teacherMessage);
            umsSetFlash("teachers", "success", "Teacher updated successfully.");
            header("Location: list.php");
            exit();
        }

        $error = umsFriendlyDbMessage("update", "teacher", sqlsrv_errors(SQLSRV_ERR_ERRORS));
    }
}
